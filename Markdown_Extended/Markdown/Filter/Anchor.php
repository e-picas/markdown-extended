...
class Markdown_Filter_Anchor extends Markdown_Filter
{
	...
	{
		if (Markdown_Extended::getVar('in_anchor')==true) return $text;
		Markdown_Extended::setVar('in_anchor', true);
		
		// First, handle reference-style links: [link text] [id]
		$text = preg_replace_callback('{
			(					                        # wrap whole match in $1
			  \[
				('.Markdown_Extended::getConfig('nested_brackets_re').')	# link text = $2
			  \]
			...
			(				                                    # wrap whole match in $1
			  ...
			  \(			                                  # literal paren
				...
				(?:
					<(.+?)>	                                # href = $3
				|
					('.Markdown_Extended::getConfig('nested_url_parenthesis_re').')	# href = $4
				)
				[ \n]*
				(			                                    # $5
				  ([\'"])	                                # quote char = $6
